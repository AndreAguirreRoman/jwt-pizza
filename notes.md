...
| Login new user<br/>(t@jwt.com, pw: test)            |         login.tsx           |        [PUT]/api/auth           |      `SELECT * FROM users WHERE email = ?`<br/> `INSERT INTO auth (token, userId) VALUES (?, ?) ...`      |
| Order pizza                                         |           menu.tsx         |           [POST]/api/order        |     `INSERT INTO order (franchiseId ,storeId, userId) VALUES (?, ?, ?);` <br/> `INSERT INTO orderItem (orderId, menuId, description, price) VALUES (?, ?, ?, ?);`          |
| Verify pizza                                        |            delivery.tsx        |          [POST]/api/order/verify         |        External service or none      |
...
| Login as franchisee<br/>(f@jwt.com, pw: franchisee) |             login.tsx       |                 [PUT]/api/auth           |      `SELECT * FROM users WHERE email = ?` <br/> `INSERT INTO auth (token, userId) VALUES (?, ?) ...  `     |
...
| Login as admin<br/>(a@jwt.com, pw: admin)           |            login.tsx        |            [PUT]/api/auth           |     ` SELECT * FROM users WHERE email = ?` <br/> `INSERT INTO auth (token, userId) VALUES `      |
